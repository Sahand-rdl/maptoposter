...
def create_poster(city, country, point, dist, output_file, orientation="portrait"):
    ...
    if orientation == "portrait":
        figsize = (12, 16)  # A2-ish portrait
    else:
        figsize = (16, 12)  # A2-ish landscape
    ...
    # 3. Plot Layers
    # Layer 1: Polygons only
    if water is not None and not water.empty:
        water_poly = water[water.geometry.type.isin(["Polygon", "MultiPolygon"])]
        if not water_poly.empty:
            water_poly.plot(ax=ax, facecolor=THEME["water"], edgecolor="none", zorder=1)

    if parks is not None and not parks.empty:
        parks_poly = parks[parks.geometry.type.isin(["Polygon", "MultiPolygon"])]
        if not parks_poly.empty:
            parks_poly.plot(ax=ax, facecolor=THEME["parks"], edgecolor="none", zorder=2)
    ...
    """
    # --- PIN (minimal, premium style) ---
    pin_lat = 40.74852175114836  # Empire State Building, NYC
    pin_lon = -73.98579314906284
    ...
    ax.scatter(
        ...
        pin_lat,
        s=120,
        c="#FF3131",  # Vibrant red
        alpha=0.20,  # Increased slightly so the "glow" is visible against the dark blue
        linewidths=0,
        zorder=8,
    )
    ...
    # Ensure correct map scaling and clean frame
    ...
    # PNG (preview / web)
    plt.savefig(output_file, dpi=300, facecolor=THEME["bg"], bbox_inches="tight")

    # PDF (print / vector)
    plt.savefig(
        output_file.replace(".png", ".pdf"),
        format="pdf",
        facecolor=THEME["bg"],
        bbox_inches="tight",
    )
